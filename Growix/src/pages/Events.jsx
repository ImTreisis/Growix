...
export default function Events() {
    ...
    const loadEvents = async () => {
      ...
      try {
        if (tab === 'liked' && user) {
          ...
          const r = await api.get('/seminars', { params: { type: 'event', limit: 100, offset: 0 } })
          ...
          const r = await api.get('/seminars', { params: { ...query, type: 'event', limit: 20, offset: 0 } })
  ...
  const EmptyState = ({ message, action }) => (
    <div className="text-center py-12">
      ...
      <h3 className="text-xl font-semibold text-dusk mb-2">No events found</h3>
  ...
  const LoadingSkeleton = () => (
    <div className="text-center py-8">
      ...
      <p className="text-cocoa/80">Loading events...</p>
    </div>
  )
  ...
  return (
    <div>
      ...
      {loading && items.length === 0 ? (
        ...
        <EmptyState 
          message={
            tab === 'liked' ? "You haven't saved any events yet. Start exploring!" :
            tab === 'organized' ? "You haven't created any events yet. Share your passion!" :
            "No events match your current filters. Try adjusting your search."
          }
          action={
            tab === 'organized' ? (
              <a href="/create-event" className="px-4 py-2 bg-dusk text-white rounded-xl">Create Event</a>
            ) : (
              <button onClick={() => { params.delete('q'); setParams(params) }} className="px-4 py-2 bg-warm3 rounded-xl">Clear Filters</button>
            )
          }
        />
      ) : (
